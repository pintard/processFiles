...
const {sortByGenderAndSurname, sortByBirthdayAndSurname, sortBySurnameDesc} = require('./sorters')
 ...
 */
module.exports = processDirectory = async inputDir => {
    const fileNames = fs.readdirSync(inputDir)
    await Promise.all(fileNames.map(async fileName => await readFile(path.resolve(inputDir, fileName), 'utf-8')))
        .then(fileBodies => {
            const data = fileBodies.join('\n')
            const records = processData(data)
            processOutput(records)
        })
}
...
const processData = data => data.split('\n').map(record => {
    const schema = new Map([['lastName', 0], ['firstName', 1], ['gender', 2], ['dateOfBirth', 3], ['favoriteColor', 4]])
    const assignSchema = (pattern, vals) => 
        Array.from(pattern.entries()).forEach(([i, key]) => schema.has(key) && schema.set(key, vals[i].trim()))
    if (record.indexOf(',') >= 0) assignSchema(csv, record.split(','))
    else if (record.indexOf('|') >= 0) assignSchema(psv, record.split('|'))
...
const processOutput = records => {
    const sortingFuncs = [sortByGenderAndSurname, sortByBirthdayAndSurname, sortBySurnameDesc]
    let output = ''
